...
process_input:
	...
	handle_not_zero:
		...
		neg  	ax 			; beacuse we want to print -bx/ax 
	...
	loop1:
		...
		loop2:
			...
			check2:
				...
				imul	bl				; (mid_point - curr_column) * valueA

				mov	bx, word ptr ds:[valueB]

				add	ax, bx				; (mid_point - curr_column) * valueA + valueB

				sub	ax, word ptr ds:[ctr_in]			; result - curr_row
...
validate_range:
						; ax = a, bx = b
	cmp	ax, -1280				; check if a < 0 then print error else pick next cond
	jl	error_range_end
	cmp	ax, 1270			; check if a > 255 then print error else next cond
	jg	error_range_end
	cmp	bx, -1280				; check if b < 0 then print error else next cond
	jl	error_range_end
	cmp	bx, 1270				; check if b > 255 then print error else next cond
	jg	error_range_end
